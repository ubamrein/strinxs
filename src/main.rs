...
unsafe fn parse_dex<R: Read>(mut f: R, reg: &str) {
    let mut buffer: Vec<u8> = vec![];
    f.read_to_end(&mut buffer).expect("Could not read dex file");
    parse_dex_buf(buffer, reg);
}

unsafe fn parse_dex_buf(buffer: Vec<u8>, reg: &str) {
    let mut config: DexHeader = mem::zeroed();
    ...
    for entry in string_table {
        ...
        let _ = r.read(&mut buf).expect("Could not read buffer");
        let se = StringEntry {
        ...
        if reg != "" {
            if se.utf16_size > 0 {
                ...
                if re.is_match(the_string) {
                    ...
                    let m = Match {
                        value: String::from(the_string),
                        origin: StringType::UTF8String,
                        desc: String::from(""),
                        class: String::from(""),
    ...
    while i < config.type_ids_size {
        ...
        let the_type =
            std::str::from_utf8(&strings[index as usize].dat).expect("type should be there");
        types.push(the_type);
        offset += 4;
        i += 1;
    ...
    while i < config.method_ids_size {
        ...
        if let Some(val) = match_table.get_mut(&(method.name_idx as usize)) {
            ...
            if let Some(val) = match_table.get_mut(&(the_proto_type.shorty_idx as usize)) {
                //val.desc = method.get_description(&types, &strings, &protos);
                let m = Match {
                    value: String::from("Proto in Method"),
                    origin: StringType::Method,
                    desc: method.get_description(&types, &strings, &protos),
                    class: method.get_classname(&types),
                    function_name: method.get_function_name(&strings),
                    argc: val.value.len() - 1,
                };
    ...
    for m in match_table {
        ...
        match m.1.origin {
            StringType::Method => {
                ...
                arg_str.pop();
                let script = format!(
                    "\tvar ret = this.{}({}); console.log(ret); return ret;",
                    m.1.function_name, arg_str
                );
                ...
                frida_script += &format!(
                    "\tvar dyn_{} = Java.use(\"{}\");\n",
                    class_string,
                    &(m.1.class.replace("/", ".").replace(";", ""))[1..]
                );
                frida_script += &format!(
                    "\tdyn_{}.{}.implementation = function({}){{ {} }};\n\n",
    ...
    if have_script {
        let mut outfile =
            std::fs::File::create(format!("hook_{}_.js", Utc::now().timestamp_millis())).unwrap();
        let _ = outfile
            .write(frida_script.as_bytes())
            .expect("Could not write frida hook");
    }
}
...
unsafe fn extract_zip<R: Read + Seek>(f: R, reg: &str) {
...
fn match_unknown_file<T: Read>(mut file: T, file_name: &str, reg: &str) {
    ...
    if re.is_match(&lossy_string) {
        ...
        println!(
            "\n\n--------\n\nfound regex in unsupported type using utf8 encoding({})",
            file_name
        );
        for the_match in matches {
            println!(
                "Match: {}, from 0x{:x} to 0x{:x}",
                the_match.as_str(),
                the_match.start(),
                the_match.end()
            );
    ...
    let u16bytes = unsafe {
        std::slice::from_raw_parts(
            zip_bytes.as_ptr() as *const u8 as *const u16,
            zip_bytes.len() / 2,
        )
    };
    ...
    if re.is_match(&lossy_string) {
        let matches = re.find_iter(&lossy_string);
        println!(
            "\n\n--------\n\nfound regex in unsupported type using utf16 encoding({})",
            file_name
        );
        for the_match in matches {
            println!(
...
pub unsafe fn fill_type_from_raw_pointer<T>(obj: &mut T, data: *const u8) {
    let obj_ptr =
        std::slice::from_raw_parts_mut(obj as *mut _ as *mut u8, std::mem::size_of::<T>());
    std::slice::from_raw_parts(data, std::mem::size_of::<T>())
        .read_exact(obj_ptr)
